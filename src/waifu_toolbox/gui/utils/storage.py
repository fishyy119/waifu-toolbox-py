...
# bindable_dataclass 通过数据描述符将定义字段的存储键名映射到下划线开头的私有字段，而 typeadpter 导出的 dataclass 会跳过这步，导致运行时取值失败
...
def _make_preferences() -> _UIPreferences:
    """从磁盘恢复偏好值。"""
    try:
        schema = _PREFERENCES_ADAPTER.validate_json(_PREFERENCES_PATH.read_text(encoding="utf-8"))
