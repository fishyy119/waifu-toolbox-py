    ...
    with ui.column().classes("w-full p-6 gap-4 max-w-3xl"):
        with ui.row().classes("items-center justify-between w-full"):
            ...
            ui.button(
                "清除已完成",
                icon="delete_sweep",
                on_click=lambda: (task_manager.clear_completed(), refresh()),
            ).props("flat")
                    ...
                    with ui.card().classes("w-full"):
                        with ui.row().classes("items-center justify-between w-full"):
                            with ui.row().classes("items-center gap-2"):
                                ...
                                ui.label(t.created_at.strftime("%H:%M:%S")).classes(
                                    "text-xs text-muted"
                                )
                        ...
                        if t.status == "running":
                            ...
                            ui.linear_progress(value=t.progress, show_value=False).classes(
                                "w-full"
                            )
                            if t.progress_text:
